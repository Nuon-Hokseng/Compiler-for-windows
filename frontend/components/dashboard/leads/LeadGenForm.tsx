...
const INIT_FORM = { targetInterest: "", keywords: "", maxProfiles: 50, model: "claude-sonnet-4-6" };
...
export function LeadGenForm({ onResults, onStart, onDiscoveryPlan }: LeadGenFormProps) {
  ...
  const { targetInterest, keywords, maxProfiles, model } = form;
  const setTargetInterest = (v: string) => setForm((p) => ({ ...p, targetInterest: v }));
  const setKeywords = (v: string) => setForm((p) => ({ ...p, keywords: v }));
  const setMaxProfiles = (v: number) => setForm((p) => ({ ...p, maxProfiles: v }));
