...
# ── Shared async helpers (used by InstagramScraper & ProfileScraper) ─

async def extract_post_owner(page, logged_in_user: str = None) -> str | None:
    """
    Standalone async post-owner extractor.
    Uses 3 strategies: meta tag, page title, link scanning.
    Shared by InstagramScraper and ProfileScraper.
    """
...
async def collect_post_urls_from_hashtag(page, hashtag: str, max_posts: int = 5) -> list[str]:
    """
    Navigate to a hashtag page, scroll to load content, collect post URLs.
    Shared by InstagramScraper and ProfileScraper.

    Returns empty list if redirected to login.
    """
    ...
    post_urls: list[str] = []
    for link in post_links[:max_posts * 2]:
        href = await link.get_attribute("href")
